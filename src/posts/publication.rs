...
pub fn enrich(session: &Session, videos: &mut [VideoContext]) {
    ...
    let published_blog = latest.and_then(|upload| blog_posts.iter().rev()
        .find(|post| post.video_id == upload.video_id)).filter(|post| post.published);
    for video in videos {
        if let Some(upload) = latest.filter(|upload| upload.privacy == crate::publish::youtube::Privacy::Public) {
            video.points.push(format!("Published full video: {} — {}", upload.title, upload.url));
        }
        if let Some(post) = published_blog {
            video.points.push(format!("Published article (use this exact URL when linking): {}", post.url));
        }
        if video.id == "longform" {
            if let Some(article) = &article {
                video.points.push(format!("Companion article: {}\n{}", article.title, article.description));
            }
        }
    }
}
...
mod tests {
    ...
    fn private_videos_and_draft_articles_do_not_become_promotional_links() {
        ...
        let mut videos = vec![VideoContext { id:"longform".into(), video_type:"horizontal".into(), title:"Video".into(), points: vec![], transcript_text: String::new() }];
        ...
        assert!(!videos[0].points.iter().any(|point| point.contains("example.com")));
        let mut published_blog = blog;
        published_blog["published"] = serde_json::json!(true);
        std::fs::write(root.join(crate::blog::POSTS_JSONL), published_blog.to_string()).unwrap();
        videos[0].points.clear();
        enrich(&session, &mut videos);
        assert!(videos[0].points.iter().any(|point| point.contains("https://example.com/blog/article")));
        assert!(!videos[0].points.iter().any(|point| point.contains("cms.example.com")));
