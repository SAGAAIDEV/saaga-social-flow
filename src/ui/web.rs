...
pub enum WebEvent {
    ...
    Approve { index: usize, value: bool },
    Validate { index: usize },
    Apply,
    Reflect,
    ...
    SaveBrief { fields: std::collections::BTreeMap<String, String> },
    /// The edited card, field by field.
    ImportPortrait { data: String },
    GenerateArtwork { fields: std::collections::BTreeMap<String, String> },
    SaveVideoBrief { fields: std::collections::BTreeMap<String, String>, apply: bool },
    GenerateVideoCopy { fields: std::collections::BTreeMap<String, String> },
    SaveYoutube { fields: std::collections::BTreeMap<String, String> },
    SaveCard { fields: std::collections::BTreeMap<String, String> },
    SelectThumbnail { id: String },
    /// The image model picked in the thumbnail pane, by OpenRouter id.
    ThumbnailModel { value: String },
...
impl WebEvent {
    ...
    pub fn into_ui_event(self) -> UiEvent {
        match self {
            ...
            WebEvent::ToggleReference { name, value } => {
                UiEvent::ToggleReference { name, value }
            }
            ...
            WebEvent::GenerateSubstack => {
                UiEvent::Action(crate::hotkeys::Action::GenerateSubstack)
            }
            ...
            WebEvent::EditBlogPrompt => {
                UiEvent::Action(crate::hotkeys::Action::EditBlogPrompt)
            }
...
impl WebPane {
    ...
    pub fn show_local(
        ...
        let Some(access) = path_url(access) else { return };
        unsafe {
            self.webview
...
fn path_url(path: &std::path::Path) -> Option<objc2::rc::Retained<objc2_foundation::NSURL>> {
    ...
    Some(objc2_foundation::NSURL::fileURLWithPath(&NSString::from_str(text)))
}
...
mod tests {
    ...
    fn video_brief_events_keep_project_scope_and_apply_intent() {
        ...
        let UiEvent::SaveVideoBrief { fields, apply } = serde_json::from_str::<WebEvent>(raw).unwrap().into_ui_event() else { panic!("wrong event") };
        assert!(apply);
        assert_eq!(fields["root"], "/tmp/project");
        assert_eq!(fields["notes"], "Source notes");
        let raw = r#"{"type":"generateVideoCopy","fields":{"root":"/tmp/project","notes":"Notes"}}"#;
        assert!(matches!(serde_json::from_str::<WebEvent>(raw).unwrap().into_ui_event(), UiEvent::GenerateVideoCopy(_)));
    }
    ...
    fn a_checkbox_message_parses_with_its_value() {
        ...
        assert_eq!(got, WebEvent::Approve { index: 2, value: true });
    }
    ...
    fn the_thumbnail_messages_parse() {
        ...
        assert_eq!(
            serde_json::from_str::<WebEvent>(r#"{"type":"selectThumbnail","id":"thumb-a"}"#)
                .unwrap(),
            WebEvent::SelectThumbnail { id: "thumb-a".into() }
        );
        assert_eq!(
            ...
            .unwrap(),
            WebEvent::ToggleReference { name: "a.jpg".into(), value: true }
        );
        let brief: WebEvent =
    ...
    fn a_malformed_keep_list_is_refused_not_guessed_at() {
        ...
        assert_eq!(
            serde_json::from_str::<WebEvent>(r#"{"type":"saveEdit","chapter":1,"spans":[]}"#)
                .unwrap(),
            WebEvent::SaveEdit { chapter: 1, spans: Vec::new() }
        );
    }
    ...
    fn approve_carries_the_value_rather_than_toggling() {
        ...
        assert_eq!(off, WebEvent::Approve { index: 1, value: false });
    }
    #[test]
    fn youtube_details_reach_the_ui_event() {
        let event: WebEvent = serde_json::from_str(r#"{"type":"saveYoutube","fields":{"title":"A video","description":"Details"}}"#).unwrap();
        let UiEvent::SaveYoutube(fields) = event.into_ui_event() else { panic!("wrong event") };
